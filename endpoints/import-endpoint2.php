        ...
        if (!empty($filename) && in_array($_FILES['import']['type'], $csv_file_mimes)) {
            ...
            foreach ($Reader as $column) {
                if ($rows++ != 1) {
                    ...
                    $policy_no = $policy;
                    // $certificate_type = $column[3];
                    if(isset($column[3])){$certificate_type = $column[3];}
                    // $certificate_type = array_key_exists(3, $column);
                    // if(!empty($column[3])){echo $column[3];}
                    if(isset($column[4])){$location_from = $column[4];}
                    if(isset($column[5])){$location_to = $column[5];}
                    $total_sum_insured = 0;
                    $total_premium = 0;
                    if(isset($column[6])){$period_of_voyage = $column[6];}
                    if(isset($column[7])){$company = $column[7];}
